    ...
    return (
        <div className={`${isVisible ? 'fixed' : 'hidden'} inset-0 z-40 bg-dark-800 bg-opacity-70 flex flex-row sm:justify-center sm:items-center`}>
            <div className={`max-w-none ${'sm:' + maxWidth} w-full bg-dark-50 dark:bg-dark-700 sm:rounded-global sm:m-4`}>
                ...
                {/* Body */}
                <div className={`${isLoading ? 'hidden' : ''} my-2 px-3 py-2`}>
                    {children}
                </div>

                {/* Loading State */}
                <div className={`${isLoading ? 'flex' : 'hidden'} flex-col justify-center items-center py-10`}>
                    <svg xmlns="http://www.w3.org/2000/svg" className="icon icon-tabler icon-tabler-loader text-dark-700 dark:text-white animation animate-spin w-10 h-10" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" fill="none" strokeLinecap="round" strokeLinejoin="round">
